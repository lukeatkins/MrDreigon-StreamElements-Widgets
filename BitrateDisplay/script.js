...
class BitrateDisplay extends DreigonWidget {

	constructor() {
		super("Bitrate Display", "dreigon-bitrate");
		this.options = {
			updateRate: 1 * 1000,
			fetchTimeout: 10000,
		}
	...
	displayBitrate(type, bitrate) {
		var message = "";
		if (type == "Offline") {
			message = this.config.bitrateOfflineText;
		} else {
			message = this.config.bitrateTextFormat;
			var bitrateFormatted = _.Formatters.float(bitrate, {decimals: this.config.formatDecimals, units: this.config.formatUnits});
			var name = type == "Primary" ? this.config.statsURLId : this.config.statsURLSecondaryId;
			message = message.split("{Name}").join(name);
			message = message.split("{Bitrate}").join(bitrateFormatted);
		}
		this.bitrateElem.innerHTML = message;
	}
